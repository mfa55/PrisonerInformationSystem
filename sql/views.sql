...
/*
returns all crimes id, type, level, date and victim's id, fname and lname
*/
CREATE VIEW crime_victims AS
SELECT c.CRIMEID, c.CRIMETYPE, c.LEVEL, c.CRIMEDATE, v.VICTIMID, v.FIRSTNAME, v.LASTNAME
FROM CRIME c JOIN VICTIM v ON c.VICTIMID=v.VICTIMID

/*
returns all crimes id, type, level, date and victim's id, fname and lname and prsioner's id, fname and lname
*/
CREATE VIEW crime_victims_with_prisoner AS
SELECT c.CRIMEID, c.CRIMETYPE, c.LEVEL, c.CRIMEDATE, v.VICTIMID, v.FIRSTNAME AS victim_firstname, v.LASTNAME AS vitcim_lastname, p.SSN, p.FIRSTNAME AS prisoner_firstname,p.LASTNAME AS prisoner_lastname
FROM CRIME c JOIN VICTIM v JOIN PRISONER p ON c.VICTIMID=v.VICTIMID and c.SSN=p.SSN
